...
void updateeyeElements(Eye &eye, const sf::Vector2f &mousePosition, bool eyeP)
{
    eye.peach.setPosition(eye.position);

    float SET_RADIUS_CIRCLE = 45;
    const sf::Vector2f deltaMouse = mousePosition - eye.position;

    sf::Vector2f circleOffsetNew = toEuclidean(SET_RADIUS_CIRCLE, eye.rotation);
    if ((fabsf(circleOffsetNew.x) >= fabsf(deltaMouse.x)) &&
        (fabsf(circleOffsetNew.y) >= fabsf(deltaMouse.y)))
    {
        if (eyeP)
        {
            eye.circle.setPosition(eye.position + deltaMouse);
        }
        else
        {
            eye.circle.setPosition(eye.position + deltaMouse);
    ...
    eye.circle.setPosition(eye.position + circleOffsetNew);
}
    ...
    Eye &rightEye)
{
    ...
    bool eyeP = true;
    updateeyeElements(leftEye, mousePosition, eyeP);
    eyeP = false;
    updateeyeElements(rightEye, mousePosition, eyeP);
}
